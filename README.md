...
For the evolution strategies algorithm to work we only have to look at the parameterized reward function $R( \mathbf{s}; \mathbf{\theta} )$, that takes a state vector and outputs a scalar reward. During the optimization process, we estimate gradients that allow us to steer the parameters $\boldmath{\theta}$ into a direction to maximize the reward $R$. Thus, we are optimizing $R$ with respect to $\mathsf{\mathbf{\theta}}$.

The evolution strategies algorithm generates at each time step a population of different parameter configurations $\mathbf \theta_i$ (the agents' neural network weights) from the base parameters $\mathbf \theta$ by adding gaussian noise ( $\mathbf \theta_i = \mathbf \theta + \mathbf \epsilon$ with $\mathbf \epsilon \sim \mathcal{N}(0, \sigma^2)$ and $i \in [1, N]$ ). After each agent has spend one episode in the environment a weighted sum over each agents policy network's parameters and gained reward is being created. This weighted sum of parameter vectors becomes the new base parameters. 
